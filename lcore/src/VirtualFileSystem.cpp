...
namespace lcore
{
    ...
    bool FileProxyPack::read(s64 offset, s64 size, void* data)
    {
        return file_->read(offset, size, data);
    }

    bool FileProxyPack::write(s64 offset, s64 size, void* data)
    {
    ...
    bool FileProxyIStream::seekg(off_t offset, int dir)
    {
    ...
    off_t FileProxyIStream::tellg()
    {
        return current_;
    }

    bool FileProxyIStream::read(s64 size, void* data)
    {
        LASSERT(0<=size);
        s64 fileSize = file_->getUncompressedSize();
        fileSize -= current_;
        if(fileSize<size){
            return false;
        }
        bool result = file_->read(current_, size, data);
        current_ += size;
        return result;
    }

    s64 FileProxyIStream::getSize()
    {
        return file_->getUncompressedSize();
    }
    ...
    FileProxy* VirtualFileSystemPack::openFile(const Char* path)
    {
        ...
        //ノード名をパスから取り出す
        path = Path::parseFirstNameFromPath(length, name_, NameBufferSize, path);
    ...
    bool FileProxyOS::read(s64 offset, s64 size, void* data)
    {
        return File::read(handle_, offset, size, data);
    }

    bool FileProxyOS::write(s64 offset, s64 size, void* data)
    {
    ...
    bool FileProxyOSRaw::read(s64 offset, s64 size, void* data)
    {
        return File::read(handle_, offset, size, data);
    }

    bool FileProxyOSRaw::write(s64 offset, s64 size, void* data)
    {
        return File::write(handle_, offset, size, data);
    }
