...
namespace
{
    ...
    bool Traversal::traverse(const Char* root)
    {
        ...
        if(Path::Exists_Directory != exists){
            ...
            fileLen = extractFileName(filename, fileLen, root);
            dirLen = extractDirectoryPath(dirpath, dirLen, root);
    ...
    bool Traversal::write(const Char* filepath)
    {
        ...
        VFSPackHeader header;
        {
            ...
            //書き出し
            if(sizeof(VFSPackHeader) != File::write(scopedHandle.handle_, sizeof(VFSPackHeader), &header)){
                return false;
            }
            s64 entrySize = sizeof(VFSData) * entries_.size();
            if(entrySize != File::write(scopedHandle.handle_, entrySize, data)){
                return false;
            }
            s64 stringSize = sizeof(Char) * stringBuffer.length();
            if(stringSize != File::write(scopedHandle.handle_, stringSize, stringBuffer.c_str())){
                return false;
            }
            bool result = true;
            s64 bufferSize = 0;
            ...
            for(s32 i=0; i<entries_.size(); ++i){
                ...
                if(size.QuadPart != File::read(entryHandle.handle_, size.QuadPart, buffer)){
                    result = false;
                    break;
                }
                if(size.QuadPart != File::write(scopedHandle.handle_, size.QuadPart, buffer)){
                    result = false;
                    break;
                }
            }
    ...
    bool readVFSPack(VFSPack& pack, const Char* filepath, bool checkHash)
    {
        ...
        DWORD entryTop = SetFilePointer(handle.handle_, 0, NULL, FILE_CURRENT);
        if(INVALID_SET_FILE_POINTER == entryTop){
            return false;
        }

        if(checkHash){
            //ハッシュチェック
            if(INVALID_SET_FILE_POINTER == SetFilePointer(handle.handle_, 0, NULL, FILE_BEGIN)){
                return false;
            }
            LARGE_INTEGER fileSize;
            ...
            if(INVALID_SET_FILE_POINTER == SetFilePointer(handle.handle_, entryTop, NULL, FILE_BEGIN)){
                return false;
            }
        }
        ...
        if(INVALID_SET_FILE_POINTER == SetFilePointer(handle.handle_, stringTop.LowPart, &stringTop.HighPart, FILE_BEGIN)){
            return false;
        }
