...
namespace lrender
{
        ...
        ,pixels_(NULL)
    {
        LASSERT(0<width);
        LASSERT(0<height);
        pixels_ = LIME_NEW Color3[width_*height_];
        weights_ = LIME_NEW f32[width_*height_];
        setFilter(filter);
    }

    Screen::~Screen()
    {
        LIME_DELETE_ARRAY(filterWeightsX_);
        LIME_DELETE_ARRAY(weights_);
        LIME_DELETE_ARRAY(pixels_);
    }
    ...
    void Screen::setFilter(Filter* filter)
    {
        ...
        LIME_DELETE_ARRAY(filterWeightsX_);
        filterWeightsX_ = LIME_NEW f32[radius*4];
