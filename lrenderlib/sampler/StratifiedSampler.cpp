...
namespace
{
    void generateStratified(f32* values, s32 numSamples, lcore::RandomWELL& random)
    {
    ...
    void generateStratified(Sample2D* values, s32 numSamples, lcore::RandomWELL& random)
    {
        f32 inv = 1.0f/numSamples;
        for(s32 i=0; i<numSamples; ++i){
    ...
    StratifiedSampler::StratifiedSampler(s32 numSamples, u32 seed)
        ...
        ,dimension2D_(0)
    {
        f32* values1D = LIME_NEW f32[numSamples_ * NumTables];
        Sample2D* values2D = LIME_NEW Sample2D[numSamples_ * NumTables];
    ...
    {
        StratifiedSampler* sampler = LIME_NEW StratifiedSampler(numSamples_, random_.rand());
