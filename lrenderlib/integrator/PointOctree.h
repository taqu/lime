    ...
    Color3 PointOctreeNode<T, U>::Mo(const AABB& bound, const Vector3& point, const U& diffusion, f32 maxSolidAngle) const
    {
        ...
        if(isLeaf_){
            ...
            for(s32 i=0; i<MaxNumNodePoints; ++i){
                ...
                f32 distance = point.distance(points_[i]->point_);

                mo += diffusion(distance) * points_[i]->E_ * points_[i]->area_;
            }
            return mo;
            ...
            if(sqrDistance<1.0e-5f || maxSolidAngle<omega){
                ...
                f32 distance = point_.distance(point);
                return diffusion(distance) * E_ * area_;
            }
        }
#else
