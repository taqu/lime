...
*/
#include "RenderSurfacePointQuery.h"
#include <lcore/CPU.h>
...
namespace
{
    ...
    bool testRaySphereBack(Intersection& intersection, const Ray& ray, const lmath::Sphere& sphere)
    {
        ...
        Vector3 center;
        sphere.getPosition(center);
        intersection.point_.muladd(t, ray.direction_, ray.origin_);

        intersection.geometricNormal_.sub(center, intersection.point_);
        intersection.geometricNormal_.normalize();
    ...
    struct JobContext
    {
        ...
        lcore::vector_arena<SurfacePoint>* surfacePoints_;
    ...
    public:
        ...
        {
            if(value.point_.distanceSqr(point_)<sqrMaxDistance_){
                result_ = true;
                return false;
            }
            return true;
    ...
    void RenderSurfacePointQuery::render()
    {
        ...
        lcore::ReadersWriterLock* RWLock = LIME_NEW lcore::ReadersWriterLock();
...
namespace
{
    void job_proc(u32 threadId, void* data)
    {
        lcore::RandomWELL random(lcore::getTime()*(threadId+1));
        ...
        lcore::vector_arena<bool> candidateRejected;
            ...
            for(s32 i=0; i<candidates.size(); ++i){
                if(candidateRejected[i]){
                    ...
                    if(query.getResult()){
                        ...
                        context.octree_->add(surfacePoint);
