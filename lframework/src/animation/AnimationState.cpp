...
namespace lfw
{
    ...
    AnimationState::~AnimationState()
    {
        LFREE(animationToJointIndex_);
    }

    void AnimationState::setClip(AnimationClip::pointer& clip)
    {
        LASSERT(NULL != skeleton_);
        if(NULL == clip){
            return;
        }
        u32 prevNumAnim = (NULL != clip_)? clip_->getNumJoints() : 0;

        clip_ = clip;

        if(prevNumAnim<clip_->getNumJoints()){
            LFREE(animationToJointIndex_);
            animationToJointIndex_ = static_cast<u8*>( LMALLOC(sizeof(u8)*clip_->getNumJoints()) );
        }

        for(u32 i=0; i<clip_->getNumJoints(); ++i){
            const Joint* joint = skeleton_->findJoint(clip->getJointAnimation(i).getName());
            if(NULL == joint){
...
namespace
{
    ...
    {
        lmath::Quaternion q = lerp(q0, q1, t);
    ...
    void AnimationState::updateBlendNone(Transform* transforms)
    {
        ...
        for(u32 i=0; i<clip_->getNumJoints(); ++i){
            ...
            const JointAnimation& jointAnim = clip_->getJointAnimation(i);

            s32 poseIndex = jointAnim.binarySearchIndex(time_);
            s32 nextPoseIndex = jointAnim.getNextIndex(time_, poseIndex);

            const JointPoseWithTime& jointPose = jointAnim.getPose(poseIndex);
            const JointPoseWithTime& nextPose = jointAnim.getPose(nextPoseIndex);
