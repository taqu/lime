...
namespace lfw
{
    ...
    void Frustum::calcInView(const Camera& camera, f32 znear, f32 zfar)
    {
        ...
        *((u32*)&f) = 0x80000000U;
        ...
        t0 = _mm_sub_ps(t0, r3);
        _mm_storeu_ps(&planes_[0].nx_, t0);

        lm128 t1 = _mm_sub_ps(r0, r3);
        _mm_storeu_ps(&planes_[1].nx_, t1);

        lm128 t2 = _mm_xor_ps(r1, mask);
        t2 = _mm_sub_ps(t2, r3);
        _mm_storeu_ps(&planes_[2].nx_, t2);

        lm128 t3 = _mm_sub_ps(r1, r3);
        _mm_storeu_ps(&planes_[3].nx_, t3);
    ...
    void Frustum::calcInWorld(const Camera& camera)
    {
        ...
        *((u32*)&f) = 0x80000000U;
        ...
        _mm_storeu_ps(&planes_[4].nx_, t4);

        lm128 t5 = _mm_sub_ps(r2, r3);
        _mm_storeu_ps(&planes_[5].nx_, t5);
#else
        //planes_[0].v_.x_ = proj.m_[3][0] + proj.m_[0][0];
        //planes_[0].v_.y_ = proj.m_[3][1] + proj.m_[0][1];
        //planes_[0].v_.z_ = proj.m_[3][2] + proj.m_[0][2];
    ...
    bool Frustum::contains(const lmath::Vector3& bmin, const lmath::Vector3& bmax) const
    {
        ...
        for(s32 i=0; i<6; ++i){
            ...
            p.z_ = (0<=planes_[i].nz_)? bmin.z_ : bmax.z_;
    ...
    {
        f32 slopeX = planes_[0].nz_ / planes_[0].nx_;
        f32 slopeY = planes_[2].nz_ / planes_[2].ny_;

        f32 halfWidth = planes_[4].d_ * slopeX;
        f32 halfHeight = planes_[4].d_ * slopeY;

        points[0].set( halfWidth,   halfHeight, planes_[4].d_, 1.0f); //右上
        points[1].set(-halfWidth,   halfHeight, planes_[4].d_, 1.0f); //左上
        points[2].set(-halfWidth,  -halfHeight, planes_[4].d_, 1.0f); //左下
        points[3].set( halfWidth,  -halfHeight, planes_[4].d_, 1.0f); //右下


        f32 zfar = -planes_[5].d_;
