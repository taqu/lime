#ifndef INC_LSCENE_TRANSIENTBUFFER_H__
#define INC_LSCENE_TRANSIENTBUFFER_H__
/**
...
#include <lcore/intrusive/RedBlackTree.h>
...
namespace lscene
{
    ...
    public:
        ...
        public:
            ...
            bool map(lgraphics::ContextRef& context, u32 subresource, lscene::MappedSubresource& mapped);
    ...
    private:
        ...
        typedef lcore::ObjectPool<Chunk, lscene::SceneAllocator> ChunkPool;

        struct RedBlackTreeAllocator
        {
            typedef lcore::RedBlackTreeNodeBase<Chunk*> node_type;
            typedef lcore::ObjectPool<node_type, lscene::SceneAllocator> NodePool;
    ...
    template<class Buffer, class Creator>
    bool TransientBuffer<Buffer, Creator>::AllocatedChunk::map(lgraphics::ContextRef& context, u32 subresource, lscene::MappedSubresource& mapped)
    {
        LASSERT(NULL != chunk_);
        if(chunk_->size_ <= 0){
            return false;
        ...
        if(chunk_->buffer_.map(context, subresource, lgraphics::MapType_WriteNoOverwrite, m)){
    ...
    template<class Buffer, class Creator>
    void TransientBuffer<Buffer, Creator>::AllocatedChunk::unmap(lgraphics::ContextRef& context, u32 subresource)
    {
        LASSERT(NULL != chunk_);
    ...
    template<class Buffer, class Creator>
    void TransientBuffer<Buffer, Creator>::initialize()
    {
    ...
    void TransientBuffer<Buffer, Creator>::begin()
    {
        ...
        while(&inGPUUse_ != chunk){
            ...
            if(false == frameSync.checkInGPUUse(chunk->event_)){
                chunk->resetInGPUUse();
                chunk->unlink();
                combine(chunk);
...
#endif //INC_LSCENE_TRANSIENTBUFFER_H__
