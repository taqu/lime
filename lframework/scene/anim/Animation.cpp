...
namespace lanim
{
    ...
    Animation::~Animation()
    {
        LIME_FREE(transforms_);
        LIME_DELETE_ARRAY(states_);
    }
    ...
    void Animation::create(s16 numClips)
    {
        ...
        LIME_DELETE_ARRAY(states_);
        numStates_ = numClips;
        states_ = LIME_NEW AnimationState[numStates_];
    }

    void Animation::setSkeleton(Skeleton::pointer skeleton)
    {
        LASSERT(skeleton);

        s32 numTransforms = (skeleton_)? skeleton_->getNumJoints() : 0;
        if(numTransforms < skeleton->getNumJoints()){
            LIME_FREE(transforms_);
            transforms_ = reinterpret_cast<Transform*>( LIME_ALIGNED_MALLOC(sizeof(Transform)*skeleton->getNumJoints(), 16) );
        }
        skeleton_ = skeleton;
    }

    void Animation::setClip(s16 index, AnimationClip* clip)
    {
