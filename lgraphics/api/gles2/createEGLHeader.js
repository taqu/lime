...
function convertHeader(in_name, out_name)
{
    ...
    for(var i=0; i<funcs.length; ++i){
        ofile.Write("LIME_EGL_FUNC(" + funcs[i].ret + ", " + funcs[i].name + ", " + funcs[i].args + ");\n");
    }

    ofile.Write("\n\n/**************************************************************/\n");
    for(var i=0; i<funcs.length; ++i){
        ofile.Write("#define " + funcs[i].name + "\t" + "LIME_EGL_FUNCPTR(" + funcs[i].name + ")\n");
    }

    ofile.Write("\n\n/**************************************************************/\n");
    for(var i=0; i<funcs.length; ++i){
        ofile.Write("LIM_EGL_GET_FUNCPTR(" + funcs[i].name + ");\n");
    }
    ifile.Close();
    ofile.Close();
}

convertHeader("egl.h", "legl.h.txt");
convertHeader("eglext.h", "leglext.h.txt");
