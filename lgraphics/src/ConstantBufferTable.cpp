...
namespace
{
    ...
    lgfx::ConstantBufferRef* ConstantBufferTable::allocate(u32 size)
    {
        ...
        if(NULL != tableEntry.top_){
            ...
            entry = LPLACEMENT_NEW( entryAllocator_.allocate() ) Entry();

            u32 allocSize = 0x01U<<(index+AllocBitShift);
            LASSERT(size<=allocSize);

            entry->buffer_ = lgfx::ConstantBuffer::create(
                allocSize,
                lgfx::Usage_Dynamic,
    ...
    lgfx::ConstantBufferRef* ConstantBufferTable::allocateLarge(u32 size)
    {
