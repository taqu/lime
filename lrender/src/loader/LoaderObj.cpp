...
#define LOG(str, ...)
    ...
    Texture::pointer LoaderObj::LoaderMtl::loadTexture(const Char* filename)
    {
        ...
        lrender::Color4* rgba = LNEW lrender::Color4[width*height];
        ...
        return Texture::pointer(LNEW Texture2D(image));
    }
    ...
    Texture::pointer LoaderObj::LoaderMtl::loadAsConstant()
    {
        ...
        return Texture::pointer(LNEW TextureConstant(rgb));
    }
    ...
    bool LoaderObj::LoaderMtl::readLine(BSDFVector& bsdfs)
    {
        ...
        if(CodeMtl_EOF == code){
            ...
            bsdfs.push_back( BSDF::pointer(LNEW Microfacet()) );

            Microfacet* brdf = static_cast<Microfacet*>(bsdfs[bsdfs.size()-1].get());
            if(1<elements_.size()){
        ...
        Microfacet* brdf = (0<bsdfs.size())? static_cast<Microfacet*>(bsdfs[bsdfs.size()-1].get()) : NULL;
    ...
    s32 LoaderObj::loadFromFile(Shape::ShapeVector& shapes, const Char* filename)
    {
        ...
        s32 dlen = lcore::extractDirectoryPath(directory_, len, filename);
        convDelimiter(directory_);
        if(0<dlen && directory_[dlen-1] != '/'){
            directory_[dlen] = '/';
            directory_[dlen+1] = CharNull;
        }
        ...
        //マテリアルがない
        if(bsdfs_.size()<=0){
            Microfacet::pointer microfacet(LNEW Microfacet());
        ...
        //shapeがない
        if(shapes_->size()<=0){
            MeshInstance::pointer meshInstance(LNEW MeshInstance(mesh_, 0, 0));
        ...
        create(points, numVertices);
        create(normals, numVertices);
        lcore::memset(&normals[0], 0, sizeof(BufferVector3::value_type)*numVertices);
        create(texcoords, numVertices);
        lcore::memset(&texcoords[0], 0, sizeof(BufferVector2::value_type)*numVertices);

        s32 numTriangles = vertices_.size()/3;
        create(triangles, numTriangles);
        ...
        case Code_UseMtl:
            ...
            if(1<elements_.size()){
                for(BSDFVector::iterator itr = bsdfs_.begin();
                    ...
                    ++itr)
                {
                    if(0 == lcore::strncmp((*itr)->getName().c_str(), elements_[1], (*itr)->getName().length())){
    ...
    void LoaderObj::pushMesh()
    {
        ...
        s32 numTriangles = vertices_.size()/3;
        shapes_->push_back(MeshInstance::pointer(LNEW MeshInstance(mesh_, numTriangles, 0)));
    }
    ...
    void LoaderObj::generateNormals(BufferVector3& normals, const BufferTriangle& triangles, const BufferVector3& positions)
    {
        ...
        for(s32 i=0; i<normals.size(); ++i){
            normals[i] = normalizeChecked(normals[i]);
        }
    }
    ...
    void LoaderObj::adjustGeometryNormals(BufferTriangle& triangles, const BufferVector3& normals, const BufferVector3& positions)
    {
        ...
        for(s32 i=0; i<triangles.size(); ++i){
            ...
            Vector3 e0 = p1-p0;
            Vector3 e1 = p2-p0;
            Vector3 ng = normalizeChecked(cross(e0,e1));

            Vector3 ns = normalizeChecked(n0+n1+n2);
