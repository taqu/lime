...
namespace lrender
{
    ...
    void RandomSampler::generate1D(lcore::RandWELL& random, f32* samples, s32 size)
    {
    ...
    void RandomSampler::generate1D(lcore::RandWELL& random, f32* samples, s32 numSamples, s32 size)
    {
    ...
    void RandomSampler::generate2D(lcore::RandWELL& random, Sample2D* samples, s32 size)
    {
        for(s32 j = 0; j < size; ++j){
    ...
    void RandomSampler::generate2D(lcore::RandWELL& random, Sample2D* samples, s32 numSamples, s32 size)
    {
        for(s32 i=0; i<size; ++i){
            for(s32 j = 0; j < numSamples; ++j){
    ...
    {
        RandomSampler* sampler = LNEW RandomSampler(numSamples_, random_.rand(), numDimensions_);
