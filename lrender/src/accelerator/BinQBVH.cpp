...
namespace lrender
{
    ...
    {
        lm128 flt_min = _mm_set1_ps(-FLT_MAX);
        lm128 flt_max = _mm_set1_ps(FLT_MAX);
        for(s32 i=0; i<3; ++i){
    ...
    {
#if LRENDER_BINQBVH_SSEPACK
        LALIGNED_FREE(packes_, PackAlign);
#endif
    ...
    void BinQBVH::build()
    {
        ...
        if(numPackesCapacity_<numPackesCapacity){
            ...
            LALIGNED_FREE(packes_, PackAlign);
            packes_ = (Primitive4Pack*)LALIGNED_MALLOC(sizeof(Primitive4Pack)*numPackesCapacity_, PackAlign);
        }
#endif
        ...
        //intersect‚Ì‚½‚ß‚ÉƒXƒ^ƒbƒN€”õ
        if(prevDepth<depth_){
            LFREE(stack_);
            stack_ = (s32*)LMALLOC(sizeof(s32)*depth_);
        }
    }
    ...
    void BinQBVH::splitBinned(s32& axis, s32& num_l, s32& num_r, AABB& bbox_l, AABB& bbox_r, f32 area, s32 start, s32 numPrimitives, const AABB& bbox)
    {
        ...
        Vector3 unit = mul((1.0f/NumBins), extent);
    ...
    bool BinQBVH::intersect(Intersection& intersection, const Ray& ray) const
    {
        ...
        while(0<=stack){
            ...
            if(Node::isLeaf(index)){
                ...
                s32 hit = lmath::testRayAABB(tminSSE, tmaxSSE, origin, invDir, raySign, node.bbox_);
    ...
    lmath::Result BinQBVH::intersectBoth(Intersection& intersection, const Ray& ray) const
    {
        lm128 origin[3];
        lm128 direction[3];
        lm128 invDir[3];
        ...
        const Shape* shape = NULL;
        lmath::Result result = lmath::Result_Fail;
        ShapePrimitive shapePrimitive;
        Ray r = ray;
        f32 u = 0.0f, v = 0.0f;
        s32 stack = 0;
        ...
        while(0<=stack){
            ...
            if(Node::isLeaf(index)){
                ...
                testPrimitivesBoth(u, v, tmaxSSE, r, origin, direction, shapePrimitive, shape, result, primIndex, Node::getNumPrimitives(index));
    ...
    lmath::Result BinQBVH::intersectBoth(const Ray& ray) const
    {
        lm128 origin[3];
        lm128 direction[3];
        lm128 invDir[3];
    ...
    void BinQBVH::pushPrimitives(Node& node, s32 index, s32 start, s32 num)
    {
        ...
        if(numPackesCapacity_<capacity){
            ...
            Primitive4Pack* packes = (Primitive4Pack*)LALIGNED_MALLOC(sizeof(Primitive4Pack)*capacity, PackAlign);
        ...
        LALIGN16 Vector4 vx[3];
        LALIGN16 Vector4 vy[3];
        LALIGN16 Vector4 vz[3];
...
#else
            s32 flag = lmath::testRayTriangleFront(
                tt, tu, tv,
                origin, direction,
                pack.x_, pack.y_, pack.z_);
#endif
    ...
    void BinQBVH::testPrimitivesBoth(
        f32& u, f32& v,
        lm128& tmaxSSE,
        Ray& ray,
        const lm128 origin[3],
        ...
        s32 start, s32 num) const
    {
        ...
        for(s32 i=0; i<num4; ++i){
            ...
            s32 flag = lmath::testRayTriangleBoth(
                tt, tu, tv,
                origin, direction,
                pack.x_, pack.y_, pack.z_);
