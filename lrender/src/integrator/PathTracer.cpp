...
namespace lrender
{
    ...
    Color3 PathTracer::Li(const Ray& r, IntegratorQuery& query)
    {
        ...
        while(query.depth_<=maxDepth_ || maxDepth_<0){
            ...
            if(strictNormal_ && 0.0f<=dot(ray.direction_, insect.geometricNormal_) * LocalCoordinate::cosTheta(wo)){
                break;
            }
            ...
            if(strictNormal_ && dot(insect.geometricNormal_, wiw)*LocalCoordinate::cosTheta(wi)<=0.0f){
                break;
            }
