...
namespace lrender
{
        ...
        Visibility& visibility) const
    {
        ...
        Vector3 o = muladd(SHADOW_EPSILON, ng, p);
        ...
        f32& pdf) const
    {
        ...
        b0 *= d0;
        b1 *= d1;
        Vector3 dir = b0+b1+direction_;
        dir *= bsphere.radius();

        Vector3 point = bsphere.position() + dir;

        ray = Ray(point, direction_, 0.0f, scene.getWorldMaxSize(), RAY_EPSILON);
        Ns = ray.direction_;
        pdf = 1.0f/(PI*bsphere.radius()*bsphere.radius());
        return radiance_;
    }
    ...
    Color3 DistantLight::power(const Scene& scene) const
    {
        lmath::Sphere bsphere;
        scene.getWorldBound().getBSphere(bsphere);
        return radiance_ * PI * bsphere.radius() * bsphere.radius();
    }
