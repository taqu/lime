...
namespace
{
    ...
    const f32 QBVHConstructor::BoundingExpansion = LMATH_F32_EPSILON*2.0f;
    ...
    QBVHConstructor::~QBVHConstructor()
    {
        LDELETE_ARRAY(vertices_);
        LDELETE_ARRAY(faces_);
    }
    ...
    bool QBVH::load(lcore::istream& in, s32 offset)
    {
        ...
        nodes_ = (Node*)LALIGNED_MALLOC(sizeof(Node)*numNodes_, 16);
        LALIGN16 f32 bbox[2][3][4];
    ...
    bool QBVH::innerTest(HitRecord& /*hitRecord*/, const lmath::Vector3& bmin, const lmath::Vector3& bmax)
    {
        ...
        bbox[0][0] = _mm_set1_ps(bmin.x_);
        bbox[0][1] = _mm_set1_ps(bmin.y_);
        bbox[0][2] = _mm_set1_ps(bmin.z_);

        bbox[1][0] = _mm_set1_ps(bmax.x_);
        bbox[1][1] = _mm_set1_ps(bmax.y_);
        bbox[1][2] = _mm_set1_ps(bmax.z_);
