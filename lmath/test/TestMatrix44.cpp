...
namespace lmath
{
    Vector4 getEyePosition(const lmath::Matrix44& view)
    {
        lmath::Vector4 eye;
    ...
    TEST_CASE("TestMatrix44::TestLookAt")
    {
        s32 resolution = 360;
        ...
        for(s32 i=0; i<(resolution*2); ++i){
            ...
            for(s32 j=0; j<4; ++j){
                for(s32 k=0; k<4; ++k){
                    EXPECT_FLOAT_EQ(invview0(j, k), invview1(j, k));
                }
            }
            ...
            for(s32 j=0; j<4; ++j){
                for(s32 k=0; k<4; ++k){
                    if(j==k){
                        EXPECT_FLOAT_EQ(identity(j,k), 1.0f);
                    }else{
                        EXPECT_FLOAT_EQ(identity(j,k), 0.0f);
                    }
                }
            }
        }
        ...
        for(s32 i=1; i<90; ++i){
            ...
            for(s32 j=0; j<4; ++j){
                for(s32 k=0; k<4; ++k){
                    EXPECT_FLOAT_NEAR(invview0(j, k), invview1(j, k), 1.0e-5f);
                }
            }
            ...
            for(s32 j=0; j<4; ++j){
                for(s32 k=0; k<4; ++k){
                    if(j==k){
                        EXPECT_FLOAT_NEAR(identity(j,k), 1.0f, 1.0e-5f);
                    }else{
                        EXPECT_FLOAT_NEAR(identity(j,k), 0.0f, 1.0e-5f);
                    }
                }
            }
            ...
            EXPECT_FLOAT_NEAR(position.x_, worldposition.x_, 1.0e-5f);
            EXPECT_FLOAT_NEAR(position.y_, worldposition.y_, 1.0e-5f);
            EXPECT_FLOAT_NEAR(position.z_, worldposition.z_, 1.0e-5f);
        }
    }

    TEST_CASE("TestMatrix44::TestPerspective")
    {
    ...
    TEST_CASE("TestMatrix44::TestInvViewProjection")
    {
        ...
        for(s32 i=1; i<90; ++i){
            ...
            for(s32 j=-10; j<10; ++j){
                ...
                for(s32 k=0; k<100; ++k){
                    ...
                    p1 /= p1.w_;
                    EXPECT_FLOAT_NEAR(p0.x_, p1.x_, 1.0e-5f);
                    EXPECT_FLOAT_NEAR(p0.y_, p1.y_, 1.0e-5f);
                    EXPECT_FLOAT_NEAR(p0.z_, p1.z_, 1.0e-5f);
                }
            }
        }//for(s32 i
    }

    TEST_CASE("TestMatrix44::TestRestoration")
    {
        lcore::RandXorshift64Star32 random;
        ...
        for(s32 i=1; i<90; ++i){
            ...
            EXPECT_FLOAT_NEAR(position.x_, worldpos.x_, 1.0e-5f);
            EXPECT_FLOAT_NEAR(position.y_, worldpos.y_, 1.0e-5f);
            EXPECT_FLOAT_NEAR(position.z_, worldpos.z_, 1.0e-5f);
        }
    }
}
