...
namespace lmath
{
    ...
    TEST_F(TestMatrix44, TestLookAt)
    {
        ...
        lmath::Vector4 eye(1.0f, 1.0f, -1.0f, 0.0f);
        ...
        lookAt(view, invview0, eye, Vector4(0.0f), Vector4::Up);
        lmath::Vector4 p = getEyePosition(view);
        EXPECT_FLOAT_EQ(p.x_, eye.x_);
        EXPECT_FLOAT_EQ(p.y_, eye.y_);
        EXPECT_FLOAT_EQ(p.z_, eye.z_);
        ...
        lmath::Vector4 position(1.0f, 1.0f, 5.0f, 1.0f);
        for(s32 i=1; i<90; ++i){
            ...
            lmath::Vector4 viewposition = mul(view, position);
            lmath::Vector4 worldposition = mul(invview0, viewposition);
    ...
    TEST_F(TestMatrix44, TestRestoration)
    {
        ...
        for(s32 i=1; i<90; ++i){
            ...
            lmath::Vector4 screenposition = mul(proj, position);
            ...
            lmath::Vector4 viewposition = mul(invproj, screenposition);
        ...
        for(s32 i=1; i<90; ++i){
            ...
            at.x_ = random.frand()*2.0f - 1.0f;
            at.w_ = 0.0f;
            lookAt(view, invview, lmath::Vector4(0.1f, 0.1f, -1.0f, 0.0f), at, lmath::Vector4::Up);
            lmath::Vector4 viewposition = mul(view, position);
            lmath::Vector4 worldposition = mul(invview, viewposition);
            EXPECT_FLOAT_EQ(position.x_, worldposition.x_);
            EXPECT_FLOAT_EQ(position.y_, worldposition.y_);
            EXPECT_FLOAT_EQ(position.z_, worldposition.z_);
        }
        ...
        for(s32 i=1; i<90; ++i){
            ...
            lmath::Vector4 viewposition = mul(invproj, screenposition);
            viewposition = mul(invview, viewposition);
            viewposition /= viewposition.w_;
            EXPECT_FLOAT_EQ(position.x_, viewposition.x_);
            EXPECT_FLOAT_EQ(position.y_, viewposition.y_);
            EXPECT_FLOAT_EQ(position.z_, viewposition.z_);
