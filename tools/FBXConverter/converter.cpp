...
namespace
{
    ...
    void convertExtension(Char* filename, s32 size, const Char* ext)
    {
        ...
        filename[e] = lcore::CharNull;
    }


    void clear(LoadMaterial& material)
    {
        material.diffuse_.one();
        material.specular_.one();
        material.ambient_.set(0.0f, 0.0f, 0.0f, lcore::calcFresnelTerm(1.33f));
        material.shadow_.set(0.0f, 0.0f, 0.0f, 0.1f);
        material.flags_ = MaterialFlag_RefractiveIndex;//MaterialFlag_CastShadow | MaterialFlag_RecieveShadow | MaterialFlag_RefractiveIndex;
        material.blendEnable_ = 0;
        material.srcBlend_ = lgfx::Blend_SrcAlpha;
        material.dstBlend_ = lgfx::Blend_InvSrcAlpha;
        material.blendOp_ = lgfx::BlendOp_Add;
        for(s32 i=0; i<TextureType_Num; ++i){
            material.textureIDs_[i] = TextureType_Invalid;
        }
    }
}
    ...
    Manager::Manager()
    {
        ...
        if(NULL != manager_){
            ...
            //ios->SetBoolProp(EXP_FBX_TEMPLATE, true);
            //ios->SetBoolProp(EXP_FBX_PIVOT, true);
            //ios->SetBoolProp(EXP_FBX_GLOBAL_SETTINGS, true);
            //ios->SetBoolProp(EXP_FBX_CHARACTER, true);
            //ios->SetBoolProp(EXP_FBX_CONSTRAINT, true);
            //ios->SetBoolProp(EXP_FBX_GOBO, true);
            //ios->SetBoolProp(EXP_FBX_SHAPE, true);
            //ios->SetBoolProp(EXP_FBX_MATERIAL, true);
            //ios->SetBoolProp(EXP_FBX_TEXTURE, true);
            //ios->SetBoolProp(EXP_FBX_MODEL, true);
            //ios->SetBoolProp(EXP_FBX_ANIMATION, true);
            //ios->SetBoolProp(EXP_FBX_EMBEDDED, true);
            //ios->SetBoolProp(EXP_FBX_PASSWORD, true);
            //ios->SetBoolProp(EXP_FBX_PASSWORD_ENABLE, true);
        }
    ...
    bool Converter::process(FbxScene* scene)
    {
        ...
        {//テクスチャ保存
            ...
            for(s32 i=0; i<count; ++i){
                ...
                if(fileTexture->GetTextureUse() == FbxFileTexture::eStandard){
                    ...
                    tex.type_ = TextureType_Normal;
                }
                tex.addressUVW_ = lgfx::TexAddress_Clamp;
                tex.filterType_ = lgfx::TexFilter_MinMagMipLinear;
                tex.compFunc_ = lgfx::Cmp_Never;
                tex.borderColor_ = 0.0f;
        ...
        {//スケルトン生成
            ...
            for(u32 i=0; i<joints_.size(); ++i){
                ...
                joints_[i].name_[0] = lcore::CharNull;
            }
            traverseJoint(root);
        }
    ...
    void Converter::pushJoint(FbxNode* node)
    {
        ...
        for(s32 j=0; j<skinCount; ++j){
            ...
            for(s32 k=0; k<skin->GetClusterCount(); ++k){
                ...
                LoadJoint& joint = joints_[boneIndex];
    ...
    s16 Converter::pushGeometry(FbxNode* node)
    {
        ...
        //TODO:セカンドUV
        if(0<mesh->GetElementUVCount()){
            getUVs(geometry, mesh, mesh->GetElementUV(0));
        }
    ...
    void Converter::pushMesh(LoadNode& dst, FbxNode* src)
    {
        ...
        if(1<numMaterialLayers){
            ...
            U16Vector* indexVectors = LNEW U16Vector[materials_.size()];
    ...
    void Converter::pushMaterial(FbxScene* scene)
    {
        for(int i=0; i<scene->GetMaterialCount(); ++i){
            ...
            if( surface->GetClassId().Is(FbxSurfacePhong::ClassId) ){
                ...
                material.flags_ |= MaterialFlag_RefractiveIndex;
    ...
    void Converter::calcSphere(LoadMesh& mesh)
    {
        GeometryCVT& geometry = geometries_[ mesh.geometry_ ];
        lmath::Vector3* buffer = LNEW lmath::Vector3[mesh.numIndices_];
...
#define PRINT3(MSG, P0, P1, P2)
    ...
    bool Converter::out(const Char* path)
    {
        ...
        for(u32 i=0; i<materials_.size(); ++i){
            write(file, materials_[i], sizeof(LoadMaterial));
        }

        for(u32 i=0; i<meshes_.size(); ++i){
            write(file, meshes_[i], sizeof(LoadMesh));
        }

        for(u32 i=0; i<nodes_.size(); ++i){
            write(file, nodes_[i], sizeof(LoadNode));
        }

        for(u32 i=0; i<joints_.size(); ++i){
            write(file, joints_[i], sizeof(LoadJoint));
        }

        for(u32 i=0; i<textures_.size(); ++i){
            write(file, textures_[i], sizeof(LoadTexture));
        }
